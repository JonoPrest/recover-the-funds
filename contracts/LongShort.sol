...
contract LongShort is ILongShort, AccessControlledAndUpgradeable {
    ...
    mapping(uint32 => mapping(bool => uint256))
        public marketSideValueInPaymentTokenLEGACY;
    ...
    mapping(uint32 => mapping(bool => uint256))
        public
        override batched_amountPaymentToken_deposit;
    mapping(uint32 => mapping(bool => uint256))
        public
        override batched_amountSyntheticToken_redeem;
    mapping(uint32 => mapping(bool => uint256))
        public
    ...
    mapping(uint32 => MarketSideValueInPaymentToken)
        public
        override marketSideValueInPaymentToken;
    ...
    mapping(uint32 => mapping(address => uint256))
        public userNextPrice_currentUpdateIndex;
    ...
    function requireMarketExistsModifierLogic(uint32 marketIndex)
        internal
        view
        virtual
    {
    ...
    function changeMarketTreasurySplitGradient(
        ...
        marketTreasurySplitGradient_e18[
            marketIndex
        ] = _marketTreasurySplitGradient_e18;
    }

    function changeMarketFundingRateMultiplier(
        uint32 marketIndex,
        uint256 _fundingRateMultiplier_e18
    ) external adminOnly {
        require(
            _fundingRateMultiplier_e18 <= 5e19,
            "not in range: funding rate <= 5000%"
        );
        fundingRateMultiplier_e18[marketIndex] = _fundingRateMultiplier_e18;
        emit MarketFundingRateMultiplerChanged(
            marketIndex,
    ...
    function createNewSyntheticMarket(
        ...
        syntheticTokens[marketIndex][true] = ITokenFactory(tokenFactory)
        ...
        syntheticTokens[marketIndex][false] = ITokenFactory(tokenFactory)
            .createSyntheticToken(
        ...
        assetPrice[marketIndex] = IOracleManager(oracleManagers[marketIndex])
            .updatePrice();
    ...
    function _seedMarketInitially(
        ...
        uint256 amountToLockInYieldManager = initialMarketSeedForEachMarketSide *
                2;
    ...
    function initializeMarket(
        ...
        require(
            marketLeverage <= 50e18 && marketLeverage >= 1e17,
            "Incorrect leverage"
        );
    ...
    function _getAmountPaymentToken(
        ...
        return
            (amountSyntheticToken * syntheticTokenPriceInPaymentTokens) / 1e18;
    }
    ...
    function _getAmountSyntheticToken(
        ...
        return
            (amountPaymentTokenBackingSynth * 1e18) /
  ...
  */
    function _getEquivalentAmountSyntheticTokensOnTargetSide(
        ...
        pure
        virtual
        returns (uint256 equivalentAmountSyntheticTokensOnTargetSide)
    {
    ...
    function get_syntheticToken_priceSnapshot(
        uint32 marketIndex,
        uint256 priceSnapshotIndex
    ) public view override returns (uint256 priceLong, uint256 priceShort) {
        priceLong = uint256(
            syntheticToken_priceSnapshot[marketIndex][priceSnapshotIndex]
                .price_long
        );
        ...
        if (HAS_LEGACY_DATA) {
            ...
            if (
                ...
                priceLong = syntheticToken_priceSnapshotLEGACY[marketIndex][
                    true
                ][priceSnapshotIndex];
                priceShort = syntheticToken_priceSnapshotLEGACY[marketIndex][
                    false
                ][priceSnapshotIndex];
            }
        }
    ...
    function get_syntheticToken_priceSnapshot_side(
        ...
        if (HAS_LEGACY_DATA) {
            // In case price requested is part of legacy data-structure
            if (price == 0) {
                price = syntheticToken_priceSnapshotLEGACY[marketIndex][isLong][
                    priceSnapshotIndex
                ];
            }
        }
    ...
    function getAmountSyntheticTokenToMintOnTargetSide(
        ...
        virtual
        override
        returns (uint256 amountSyntheticTokensToMintOnTargetSide)
    {
  ...
  */
        ...
        returns (uint256 confirmedButNotSettledBalance)
    {
        ...
        if (
            ...
            uint256 amountPaymentTokenDeposited = userNextPrice_paymentToken_depositAmount[
                    marketIndex
                ][isLong][user];
   ...
   */
        ...
        returns (bool isLongSideUnderbalanced, uint256 treasuryYieldPercent_e18)
    {
        ...
        uint256 marketPercentCalculated_e18 = (imbalance *
            marketTreasurySplitGradient_e18[marketIndex]) /
    ...
    */
    ...
    function _claimAndDistributeYieldThenRebalanceMarket(
        ...
        MarketSideValueInPaymentToken
            storage currentMarketSideValueInPaymentToken = marketSideValueInPaymentToken[
                marketIndex
            ];
        ...
        requireMarketExists(marketIndex)
    {
        ...
        int256 newAssetPrice = IOracleManager(oracleManagers[marketIndex])
            .updatePrice();
        ...
        if (assetPriceHasChanged) {
            ...
            uint256 syntheticTokenPrice_inPaymentTokens_long = _getSyntheticTokenPrice(
                    newLongPoolValue,
                    ISyntheticToken(syntheticTokens[marketIndex][true])
                        .totalSupply()
                );
            uint256 syntheticTokenPrice_inPaymentTokens_short = _getSyntheticTokenPrice(
                    newShortPoolValue,
                    ISyntheticToken(syntheticTokens[marketIndex][false])
                        .totalSupply()
                );
    ...
    function _mintNextPrice(
        ...
        internal
        virtual
        updateSystemStateMarketAndExecuteOutstandingNextPriceSettlements(
            msg.sender,
            marketIndex
        )
        gemCollecting
    {
        ...
        userNextPrice_currentUpdateIndex[marketIndex][
            msg.sender
        ] = nextUpdateIndex;
        ...
        gemCollecting
    {
        ...
        IStaker(staker).mintAndStakeNextPrice(
            marketIndex,
            amount,
            isLong,
            msg.sender
    ...
    function redeemLongNextPrice(uint32 marketIndex, uint256 tokens_redeem)
        external
        override
    {
    ...
    /// @param marketIndex An uint32 which uniquely identifies a market.
    /// @param tokens_redeem Amount in wei of synth tokens to redeem at the next oracle price.
    function redeemShortNextPrice(uint32 marketIndex, uint256 tokens_redeem)
        external
        override
    {
        ...
        gemCollecting
    {
        ...
        ISyntheticToken(syntheticTokens[marketIndex][isShiftFromLong])
            .transferFrom(
                msg.sender,
                address(this),
        ...
        userNextPrice_syntheticToken_toShiftAwayFrom_marketSide[marketIndex][
            isShiftFromLong
        ][msg.sender] += amountSyntheticTokensToShift;
        uint256 nextUpdateIndex = marketUpdateIndex[marketIndex] + 1;
        userNextPrice_currentUpdateIndex[marketIndex][
            msg.sender
        ] = nextUpdateIndex;
    ...
    function shiftPositionFromLongNextPrice(
        uint32 marketIndex,
        uint256 amountSyntheticTokensToShift
    ) external override {
    ...
    function shiftPositionFromShortNextPrice(
        uint32 marketIndex,
        uint256 amountSyntheticTokensToShift
    ) external override {
        shiftPositionNextPrice(
            marketIndex,
            amountSyntheticTokensToShift,
            false
        );
    ...
    function _executeOutstandingNextPriceTokenShifts(
        ...
        if (syntheticToken_toShiftAwayFrom_marketSide > 0) {
            ...
            userNextPrice_syntheticToken_toShiftAwayFrom_marketSide[
                marketIndex
            ][isShiftFromLong][user] = 0;

            require(
                ISyntheticToken(syntheticTokens[marketIndex][!isShiftFromLong])
                    .transfer(user, syntheticToken_toShiftTowardsTargetSide)
            );
        }
    }
    ...
    function _executeOutstandingNextPriceSettlements(
        ...
        uint256 userCurrentUpdateIndex = userNextPrice_currentUpdateIndex[
            marketIndex
        ][user];
        if (
            userCurrentUpdateIndex != 0 &&
            userCurrentUpdateIndex <= marketUpdateIndex[marketIndex]
        ) {
            //
            /*
        Hello!
        ...
        if (marketSideValueInPaymentToken[1].value_long == 0) {
            for (uint32 market = 1; market <= latestMarket; market++) {
                marketSideValueInPaymentToken[
                    ...
                    SafeCast.toUint128(
                        marketSideValueInPaymentTokenLEGACY[market][true]
                    ),
                    SafeCast.toUint128(
                        marketSideValueInPaymentTokenLEGACY[market][false]
                    )
                );
            }
        }
